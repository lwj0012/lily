...
public class cccDlg extends Activity implements OnClickListener {
	userinfo mUserinfo;
	String sourceString = "";
	String fileString = "";
	@Override
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.ccc_dlg);
        setTitle("快速转载");
        Bundle mBundle = getIntent().getExtras();
        sourceString = mBundle.getString("source");
        fileString = mBundle.getString("file");
        mUserinfo = (userinfo) getApplication();
        ArrayAdapter<String> adapter = new ArrayAdapter<String>(this,
                android.R.layout.simple_dropdown_item_1line, boards);
        AutoCompleteTextView textView = (AutoCompleteTextView) findViewById(R.id.ccc_dlg_edit);
        textView.setAdapter(adapter);
        Button mButton = (Button)findViewById(R.id.ccc_dlg_go);
	...
	private void ccThread(String board) {
		String code = mUserinfo.getCode();
		String cookie = mUserinfo.getCookies();
		String newurlString = "http://bbs.nju.edu.cn/" + code + "/bbsccc";
		HttpPost httpRequest = new HttpPost(newurlString);
	    ArrayList<NameValuePair> postData = new ArrayList<NameValuePair>();
	    postData.add(new BasicNameValuePair("board", sourceString));
	    postData.add(new BasicNameValuePair("file", fileString));
	    postData.add(new BasicNameValuePair("target", board));
		...
		if (httpResponse.getStatusLine().getStatusCode() == 200) {
			Toast.makeText(getApplicationContext(), "转帖成功！",
